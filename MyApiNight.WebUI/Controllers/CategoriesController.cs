...
namespace MyApiNight.WebUI.Controllers
{
    public class CategoriesController : Controller
    {
        ...
        public async Task<IActionResult> CreateCategory(CreateCategoryDto createCategoryDto)
        {
            ...
            var reponseMessage = await client.PostAsync("https://localhost:7162/api/Category", stringContent);
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.GetAsync("https://localhost:7162/api/Category/GetCategory?id=" + id);
            if (responseMessage.IsSuccessStatusCode)
            {
                var jsonData = await responseMessage.Content.ReadAsStringAsync();
